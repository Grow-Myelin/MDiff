...
if (!apiKey) {
  try {
    ...
    console.error('No API key found. Please set CEREBRAS_API_KEY environment variable or create .env file');
    process.exit(1);
  }
}
...
// Function to perform a single analysis pass with retry mechanism
async function performAnalysisPass(fileContents, passNumber, previousResults = null) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    try {
        ...
        messages: [
          { 
            ...
            content: `You are an expert document analyzer performing pass ${passNumber} of a multi-pass extraction. 
                     ...
                     CRITICAL: You MUST return a complete, valid JSON response. Do not truncate or cut off the response.
                     Ensure all JSON objects and arrays are properly closed with closing braces and brackets.
                     If you reach token limits, reduce the number of items extracted but ensure the JSON is complete.`
          },
          { 
            role: "user", 
            content: prompt 
      ...
      console.log(`✓ Pass ${passNumber} completed successfully on attempt ${attempt}`);
      return result;
      
    } catch (error) {
      console.error(`✗ Pass ${passNumber} attempt ${attempt} failed:`, error.message);
      
      if (attempt === 3) {
        console.error(`All retry attempts failed for pass ${passNumber}`);
...
async function main() {
  try {
    ...
    // Read markdown files from current directory
